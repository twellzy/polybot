...
export class TradeExecutor {
	...
	async initialize(): Promise<void> {
		...
		console.log(`   Signing wallet (EOA): ${config.funderWallet}`);
		const funderAddress = config.funderWallet;
		console.log(`   Funder wallet: ${funderAddress}`);
		console.log(`   Signature type: 1`);
	...
	calculateCopySize(originalSize: number): number {
		const {tradeCeiling, positionSizeMultiplier, maxTradeSize, minTradeSize, orderType } = config.trading;
		const marketMin = orderType === 'FOK' || orderType === 'FAK' ? 1 : minTradeSize;

		let size = (originalSize / tradeCeiling) * (maxTradeSize - minTradeSize);

		size = Math.min(maxTradeSize, minTradeSize+size);
	...
	private async executeLimitOrder(originalTrade: Trade, copyNotional: number): Promise<CopyExecutionResult> {
		...
		const validatedPrice = originalTrade.side === 'BUY' ? await this.validatePrice(marketPrice, originalTrade.tokenId): originalTrade.price;
		const copyShares = originalTrade.side === 'BUY' ? this.calculateSharesFromNotional(copyNotional, validatedPrice): await this.getHeldTokens(originalTrade.tokenId);

		await this.validateBalance(originalTrade.side === 'BUY' ? copyNotional: copyShares, originalTrade.tokenId, originalTrade.side);

		console.log(`   Limit price: ${validatedPrice.toFixed(4)}`);
		console.log(`   Copy shares: ${copyShares.toFixed(4)}`);
	...
	// Has problems denying token selling due to insufficient balance
	// Needs a better way to check user held positions
	// issue with pre-defined clob functions no longer existing 
	//    |
	//    |
	//    V


	//SHOULD NOW BE FIXED REQUIRES TESTING
	//++++++++++++++++++++++++++++


	async validateBalance(requiredAmount: number, tokenId: string, side: string): Promise<void> {
		try {
			const metadata = await this.getMarketMetadata(tokenId);
			...
			if (side === 'BUY'){
				const required = ethers.utils.parseUnits(requiredAmount.toFixed(4).toString(), decimals);
				const balance = await usdc.balanceOf(config.funderWallet);
				...
				const allowanceCtf = await usdc.allowance(config.funderWallet, config.contracts.ctf);
				...
				const allowanceEx = await usdc.allowance(config.funderWallet, exchangeAddress);
			...
			const clobBal = side === 'BUY' ? await this.clobClient.getBalanceAllowance({ asset_type: AssetType.COLLATERAL }): await this.clobClient.getBalanceAllowance({ asset_type: AssetType.CONDITIONAL, token_id: tokenId})
			const clobBalance = side === 'BUY' ? parseFloat(clobBal?.balance || '0') / 1_000_000: parseFloat(clobBal?.balance || '0');
			...
			// This line just always complains it is 'allowances' not 'allowance' I think the language server's API is out of date
			...
			const approved = await ctf.isApprovedForAll(config.funderWallet, exchangeAddress);
			if (!approved) {
	...
	async getHeldTokens(tokenId: string): Promise<number>{
		const heldTokens = await this.clobClient.getBalanceAllowance({ asset_type: AssetType.CONDITIONAL, token_id: tokenId});
		const heldShares = ethers.utils.formatUnits(heldTokens.balance || '0', 6);

		return parseFloat(heldShares);
	}
